...
CREATE OR REPLACE FUNCTION new_partition_creator() RETURNS trigger AS
  $BODY$
   ...
   BEGIN
	...
	INSERT into ip2hostname SELECT distinct  lower(split_part(message,' ', 3)) as IP, split_part(message,' ', 5) as DNS FROM systemevents where syslogtag like 'dnsmasq%' and message like ' /etc/hosts%' and devicereportedtime > now() - interval '1' day and isnumeric(left(split_part(message,' ', 3),1));
